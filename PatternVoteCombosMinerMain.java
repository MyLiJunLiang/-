...
/**
 * PatternVoteCombosMinerMain v1 (two-stage TopM -> combo search)
 *
 * ✅ 你要的能力（基于 SRMF 交易日志）：
 ...
 *
 * 运行示例：
 *   java eval.PatternVoteCombosMinerMain --tradeLog=./out/trade_log.txt --db=./okx_candles_osc.db --instId=ETH-USDT-SWAP --bar=30m --topN=10 --lookback=12 --minBinN=50 --minAgreeN=100
 */
public class PatternVoteCombosMinerMain {
...
static final class ComboSpec {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int topN = 50;
        int lookback = 1; // patterns默认只看本根(无未来函数)
        int maxEvents = 0;
        int minBinN = 30;     // patterns较稀疏，默认降低门槛     // 你建议 50
        int minAgreeN = 50;  // agree 样本 < N：不进入 TopN（仍写 CSV）  // 你建议 ≥100 才进榜/才硬过滤
        int randCombos = 400000; // 两阶段组合采样数量 // 两阶段组合采样数量
        int comboMinK = 1;
        int comboMaxK = 3;
        long randSeed = 1L;
        int topM = 80; // patterns池很小，80 足够覆盖全部
        int familyCap = 80; // patterns只有 CANDLE/PS_DT 两个 family，默认不限制
        String topMMetric = "hybrid";
        String comboSearch = "random";
        int beamWidth = 60;

        int failMonthCombos = 1; // 失败月+总体提升：随机组合数量（建议 20万~100万）
        ...
        int oos6 = 1;
        int oos12 = 3;
        double costPct = 0.0010; // 单边0.0005 -> 双边0.0010（用于 Fixed-R 统计） // fee+slippage, applied on |PnL|
     // topM|all
        ...
        String fm = (cfg.featureMode == null ? "none" : cfg.featureMode.trim().toLowerCase());
        ...
        System.out.println("========================================================");
        System.out.println("PatternVoteCombosMinerMain v1 (two-stage TopM -> combo search)（区分度 + 胜率影响 + 盈利影响）");
        ...
        // 仅保留“形态/结构”类（你这次要求：只挖掘形态或结构投票组合）
        // 约定：在 ConditionFactory-1 里，这些形态被 tag.apply(CANDLE/PS_DT, ...) 包装，
        // 因此 id() 形如：F=CANDLE|HAMMER 或 F=PS_DT|DOUBLE_TOP ...
        List<PoolCondition> conds = new ArrayList<>();
        for (PoolCondition c : condsAll) {
            String id = c.id();
            if (id != null && (id.startsWith("F=CANDLE|") || id.startsWith("F=PS_DT|"))) {
                conds.add(c);
            }
        }
        System.out.println("指标库单指标数量(all)：" + condsAll.size());
        System.out.println("形态/结构指标数量(仅CANDLE/PS_DT)：" + conds.size());
        if (conds.isEmpty()) {
            throw new IllegalStateException("未在指标库中发现形态/结构类条件（F=CANDLE| 或 F=PS_DT|）。" +
                    "请确认你已将 16 个单形态 + 双顶双底写入 ConditionFactory，并使用 tag.apply(CANDLE/PS_DT, ...) 包装。");
        }
